...
export default function UserQueueStatus() {
  ...
  return <main className="max-w-[1280px] mx-auto px-4 md:px-10 py-8 space-y-8 w-full">
      {/* Hero Header */}
      <div className="space-y-2 pt-4">
        <h2 className="text-3xl font-semibold text-on-surface">{t('liveQueue')}</h2>
        <p className="text-base text-on-surface-variant max-w-2xl">{t("trackYourQueuePositionAndWaiti")}</p>
      </div>
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <span className="bg-primary-container/10 text-primary-container px-3 py-1 rounded-full text-sm font-bold uppercase tracking-wider">{t("inQueue")}</span>
              ...
              <div className="bg-surface-container border border-primary/10 rounded-xl p-6 min-w-[250px] shadow-sm">
                <div className="grid grid-cols-2 gap-4 text-center">
                  <div>
                    ...
                    <p className="text-[11px] text-on-surface-variant font-medium mt-1 uppercase tracking-wider">{t("devoteesAhead")}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-on-surface-variant font-medium mt-1 uppercase tracking-wider">{t("tokensAhead")}</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-on-surface">{queueInfo.userPersons}</p>
                    <p className="text-[11px] text-on-surface-variant font-medium mt-1 uppercase tracking-wider">{t("yourGroup")}</p>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button className="flex items-center justify-center gap-3 bg-primary text-white py-4 rounded-xl font-bold hover:bg-primary/90 transition-all shadow-md active:scale-95">
              <span className="material-symbols-outlined">{t("qrcode2")}</span>
              {t('viewPass')}
            </button>
            ...
            <a href="https://www.google.com/maps/place/ardhanareshwari+nag+jotirling/@20.6836728,73.7838213,17z/data=!3m1!4b1!4m6!3m5!1s0x3bde3de27d6c9e1d:0x42fcd5a79fa923be!8m2!3d20.6836728!4d73.7864016!16s%2Fg%2F11b7jjr46p?hl=en-IN&entry=ttu&g_ep=EgoyMDI2MDYyMy4wIKXMDSoASAFQAw%3D%3D" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-3 bg-white border-2 border-primary/20 text-primary py-4 rounded-xl font-bold hover:bg-primary/5 transition-all active:scale-95">
              <span className="material-symbols-outlined">near_me</span>{t("openDirections")}</a>
          </div>
              ...
              <h3 className="text-2xl font-bold text-on-surface">{t("noActiveDarshanPass")}</h3>
              <p className="text-on-surface-variant">{t("youDontHaveAnActiveDarshanPass")}</p>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-inverse-surface text-surface rounded-xl p-6 shadow-lg space-y-6">
            <div className="flex items-center gap-2">
              ...
              <h4 className="text-sm font-bold uppercase tracking-widest text-primary-fixed">{t("liveCounters")}</h4>
            </div>
            
            <div className="flex justify-between items-end border-b border-surface-variant/10 pb-6">
              <div>
                <p className="text-surface-variant text-sm">{t("activeToken")}</p>
            ...
            <div className="space-y-4">
              <p className="text-xs font-bold text-surface-variant uppercase">{t("nextUpcoming")}</p>
              <div className="flex flex-wrap gap-2">
                {queueInfo.nextTokens.length > 0 ? queueInfo.nextTokens.map((token, i) => <span key={i} className="bg-surface-variant/10 border border-surface-variant/20 px-3 py-1.5 rounded-lg text-sm font-semibold">{token}</span>) : <span className="text-surface-variant">{t("none")}</span>}
              </div>
            </div>
            ...
            <div className="bg-surface-container-highest/10 rounded-lg p-4 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm">{t("estimatedDarshanTime")}</span>
              </div>
              <span className="font-bold text-white">{t("1045Am")}</span>
            </div>
          </div>
          ...
          <div className="bg-surface-container-lowest border border-outline/10 rounded-xl p-6 shadow-sm space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="text-sm font-bold text-on-surface">{t("queueMetrics")}</h4>
              <span className="text-[10px] text-on-surface-variant flex items-center gap-1">
                <span className="material-symbols-outlined text-[12px]">sync</span>{t("updated1MinAgo")}</span>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-surface-container-low rounded-lg">
                <p className="text-[10px] uppercase font-bold text-on-surface-variant">{t("queueSpeed")}</p>
                <p className="text-lg font-bold text-primary">1.5 <span className="text-xs font-medium">{t("tknsm")}</span></p>
              </div>
              <div className="p-3 bg-surface-container-low rounded-lg">
                <p className="text-[10px] uppercase font-bold text-on-surface-variant">{t("avgTime")}</p>
          ...
          <div className="bg-tertiary-fixed text-on-tertiary-fixed rounded-xl p-6 shadow-sm relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <h4 className="font-bold">{t("templeNotice")}</h4>
            </div>
            <p className="text-sm leading-relaxed mb-4">{t("mahaaartiScheduledFor1130AmTod")}</p>
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-xs font-medium">
                <span className="material-symbols-outlined text-sm">history</span>{t("templeHours400Am1000Pm")}</div>
              <div className="flex items-center gap-2 text-xs font-medium">
                <span className="material-symbols-outlined text-sm">call</span>{t("helpline911234567890")}</div>
            </div>
          </div>

          {/* Temple Map Snippet */}
          <div className="bg-surface-container-lowest border border-outline/10 rounded-xl p-4 shadow-sm">
            <div className="h-32 w-full rounded-lg mb-3 bg-cover bg-center overflow-hidden" style={{
            backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuCsFT3IFfq63Xzd8M_ArrBfFX7jyZMTp2IHBjaNMtTRZBNuemS8ZpmEmSbKR8VURvlZHxa0gWKEBeROxiq-_plqaAi9wH-aBOkeQrltkexzlxWbMHye7YasrstqyXOSszFzL85_wP2W7280jLISnD-0X6BTksHlSH8VezPrJemCgmN75bs7OD8GqCDEgDgD_kGdakZjSWVCXAMgfiBfq64ffLBNFgLBeiJTNt-Lv6kTPVCv6GpgsPon53fYxKd2OrcOhEerxJ8ap4k')"
          }}></div>
            <div className="flex justify-between items-center">
              <div>
                <p className="text-xs font-bold text-on-surface">{t("queueHallB")}</p>
                <p className="text-[10px] text-on-surface-variant">{t("currentLocationNearEntranceGat")}</p>
              </div>
