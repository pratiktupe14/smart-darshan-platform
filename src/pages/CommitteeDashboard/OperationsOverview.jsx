...
export default function OperationsOverview() {
  ...
  return <div className="max-w-7xl mx-auto w-full flex flex-col gap-8">
      {/* Summary Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 rounded-xl flex flex-col gap-1 relative overflow-hidden group">
          <div className="absolute -right-4 -top-4 text-primary/10 group-hover:text-primary/20 transition-colors">
            <span className="material-symbols-outlined text-7xl">{t("qrcode2")}</span>
          </div>
          ...
          <div className="mt-2 flex items-center gap-1 text-on-surface-variant text-xs font-medium">
            <span>{t("digitalEntryActive")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="glass-card p-6 rounded-xl flex flex-col gap-1 relative overflow-hidden group">
          ...
          <p className="text-on-surface-variant text-xs font-bold uppercase tracking-wider font-label-sm">{t("todaysBookings1")}</p>
          <p className="text-on-surface text-3xl font-extrabold">{bookingsToday?.toLocaleString() || 0}</p>
          <div className="mt-2 flex items-center gap-1 text-on-surface-variant text-xs font-medium">
            <span>{t("totalDarshanBookingsCreatedTod")}</span>
          </div>
        </div>
        ...
        <div className="glass-card p-6 rounded-xl flex flex-col gap-1 relative overflow-hidden group">
          ...
          <div className="mt-2 flex items-center gap-1 text-on-surface-variant text-xs font-medium">
            <span>{t("totalDevoteesCurrentlyInside")}</span>
          </div>
        </div>
        ...
        <div className="glass-card p-6 rounded-xl flex flex-col gap-1 relative overflow-hidden group">
          ...
          <p className="text-on-surface-variant text-xs font-bold uppercase tracking-wider font-label-sm">{t("pendingEntries")}</p>
          <p className="text-on-surface text-3xl font-extrabold">{totalPendingEntries?.toLocaleString() || 0}</p>
          <div className="mt-2 flex items-center gap-1 text-on-surface-variant text-xs font-medium">
            <span>{t("bookedButNotYetEntered")}</span>
          </div>
        </div>
      </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Link to="/dashboard/committee/scanner" className="flex flex-col items-center justify-center gap-3 p-6 bg-primary text-white rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all transform hover:-translate-y-1 text-center">
            <span className="material-symbols-outlined text-3xl">qr_code_scanner</span>
            <span className="text-sm font-bold">{t('scanQrCode')}</span>
          </Link>
          
          <Link to="/dashboard/committee/scanner-verification" className="flex flex-col items-center justify-center gap-3 p-6 bg-secondary text-white rounded-xl shadow-lg shadow-secondary/20 hover:bg-secondary/90 transition-all transform hover:-translate-y-1 text-center">
            <span className="material-symbols-outlined text-3xl">person_add</span>
            <span className="text-sm font-bold">{t('newEntry')}</span>
          </Link>
          
          <Link to="/dashboard/committee/parking" className="flex flex-col items-center justify-center gap-3 p-6 bg-surface-container-high border border-outline-variant text-on-surface rounded-xl hover:bg-surface-variant transition-all transform hover:-translate-y-1 text-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 glass-card rounded-xl p-6">
          ...
          <div className="flex flex-col gap-4">
            ...
            return <div key={index} className={`flex items-center gap-4 p-3 hover:bg-surface-container-low rounded-lg transition-colors border-l-4 ${colorClass.split(' ')[2]}`}>
                    ...
                    <div className="flex flex-col flex-grow">
                      ...
                      <p className="text-on-surface-variant text-xs">{activity.gate} • {new Date(activity.time).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit'
                  })}</p>
                    </div>
                ...
                <p className="text-on-surface-variant text-sm font-medium">{t("noRecentActivities")}</p>
              </div>}
          </div>
        </div>
        ...
        <div className="glass-card rounded-xl p-6 bg-surface-container">
          ...
          <div className="flex flex-col gap-5">
            ...
            if (ann.priority === 'high' || ann.priority === 'critical') colorClass = 'bg-error';else if (ann.priority === 'medium') colorClass = 'bg-secondary';
            return <div key={index} className="p-4 bg-white rounded-lg border border-outline-variant shadow-sm relative overflow-hidden">
                ...
                <p className="text-on-surface-variant text-sm font-medium">{t("noAnnouncementsAvailable")}</p>
              </div>}
            
            <button onClick={() => setIsAddNoticeModalOpen(true)} className="w-full py-3 bg-white border border-dashed border-primary/40 rounded-lg text-primary text-sm font-bold hover:bg-primary-container/10 transition-colors">{t("addNewNotice")}</button>
          </div>
        </div>
      </div>

      {/* Add New Notice Modal */}
      {isAddNoticeModalOpen && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl border border-outline-variant animate-in fade-in zoom-in-95 duration-200 text-on-surface">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">add_alert</span>{t("addNewNotice1")}</h3>
              <button onClick={() => setIsAddNoticeModalOpen(false)} className="text-on-surface-variant hover:bg-surface-container rounded-full p-1 transition-colors">
            ...
            <form onSubmit={handleCreateAnnouncement} className="space-y-4">
              <div>
                ...
                <input type="text" required value={newNoticeTitle} onChange={e => setNewNoticeTitle(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-4 py-2.5 outline-none focus:border-primary transition-all text-sm font-semibold" placeholder={t("enterNoticeTitle")} />
              </div>

              <div>
                <label className="block text-xs font-bold text-on-surface-variant mb-2">{t("description")}</label>
                <textarea required rows="3" value={newNoticeDesc} onChange={e => setNewNoticeDesc(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-4 py-2.5 outline-none focus:border-primary transition-all text-sm font-semibold resize-none" placeholder={t("enterNoticeDetails")}></textarea>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-on-surface-variant mb-2">{t("priority")}</label>
                  <select value={newNoticePriority} onChange={e => setNewNoticePriority(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-3 py-2.5 outline-none focus:border-primary transition-all text-sm font-semibold">
                ...
                <div>
                  ...
                  <select value={newNoticeCategory} onChange={e => setNewNoticeCategory(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-3 py-2.5 outline-none focus:border-primary transition-all text-sm font-semibold">
                    <option value="general">{t("general")}</option>
                    <option value="darshan">{t("darshan")}</option>
              ...
              <div className="pt-4 flex gap-3 justify-end">
                <button type="button" onClick={() => setIsAddNoticeModalOpen(false)} className="px-4 py-2 border border-outline-variant rounded-lg hover:bg-surface-container-low text-xs font-bold text-on-surface-variant transition-colors">
                  Cancel
                </button>
                <button type="submit" disabled={isSubmittingNotice} className="px-5 py-2 bg-primary text-white rounded-lg hover:brightness-110 text-xs font-bold transition-all shadow-md disabled:opacity-50">
