...
export default function OfflineVerification() {
  ...
  const handlePersonsChange = val => {
    ...
    setFormVisitors(prev => {
      if (prev.length < nextVal) {
        return [...prev, ...Array.from({
          length: nextVal - prev.length
        }, () => ({
          name: '',
          age: ''
        }))];
  ...
  const handleSearch = e => {
    ...
    const found = entries.find(item => item.token.toLowerCase().includes(query) || item.mobile.includes(query) || item.name.toLowerCase().includes(query));
    if (found) {
    ...
    try {
      const bookingResponse = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api'}/bookings`, {
        ...
        body: JSON.stringify({
          ...
          persons: parseInt(formPersons) || 1,
          visitors: formVisitors,
          vehicleType: formVehicleType === 'None / Walk-in' ? 'none' : formVehicleType === 'Two Wheeler' ? 'two_wheeler' : formVehicleType === 'Four Wheeler' ? 'four_wheeler' : 'bus',
          vehicleNumber: formVehicleNumber,
  ...
  const handleMarkEntered = () => {
    ...
    setEntries(prev => prev.map(item => item.token === currentDevotee.token ? {
      ...item,
      status: 'Entered'
    } : item));
  ...
  return <div className="p-container-padding-desktop space-y-stack-lg max-w-7xl mx-auto w-full animate-in fade-in duration-200">
      {/* Page Header */}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-gutter">
          ...
          <div className="bento-card p-stack-md rounded-lg">
            ...
            <form onSubmit={handleSearch} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-end">
              <div className="space-y-1">
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2 text-body-md focus:ring-primary focus:border-primary outline-none" placeholder={t("eg9876543210OrA1024")} type="text" />
              </div>
              <button type="submit" className="bg-[#82542b] text-white px-6 py-2 h-10 rounded-lg font-label-md flex items-center justify-center gap-2 hover:opacity-90 transition-opacity active:scale-[0.98]">
          ...
          <div className="bento-card p-stack-md rounded-lg relative overflow-hidden">
            ...
            <form onSubmit={handleCreateToken} className="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
              <div className="space-y-1">
                ...
                <input value={formName} onChange={e => setFormName(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2.5 text-body-md focus:ring-primary focus:border-primary outline-none" placeholder={t("enterDevoteeName")} type="text" required />
              </div>
              <div className="space-y-1">
                <label className="text-label-sm font-label-sm text-on-surface-variant">{t('mobileNumberRequired')}</label>
                <input value={formMobile} onChange={e => setFormMobile(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2.5 text-body-md focus:ring-primary focus:border-primary outline-none" placeholder={t("10digitMobileNumber")} type="tel" pattern="[0-9]{10}" required />
              </div>
              <div className="space-y-1">
                <label className="text-label-sm font-label-sm text-on-surface-variant">{t('placeVillage')}</label>
                <input value={formVillage} onChange={e => setFormVillage(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2.5 text-body-md focus:ring-primary focus:border-primary outline-none" placeholder={t("whereAreTheyFrom")} type="text" />
              </div>
              <div className="space-y-1">
                <label className="text-label-sm font-label-sm text-on-surface-variant">{t('numberOfPersons')}</label>
                <input value={formPersons} onChange={e => handlePersonsChange(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2.5 text-body-md focus:ring-primary focus:border-primary outline-none" min="1" type="number" required />
              </div>
              <div className="space-y-1">
                <label className="text-label-sm font-label-sm text-on-surface-variant">{t('vehicleType')}</label>
                <select value={formVehicleType} onChange={e => setFormVehicleType(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2.5 text-body-md focus:ring-primary focus:border-primary outline-none">
              ...
              <div className={`space-y-1 transition-all ${formVehicleType === 'None / Walk-in' ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <input value={formVehicleNumber} onChange={e => setFormVehicleNumber(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2.5 text-body-md focus:ring-primary focus:border-primary outline-none uppercase" placeholder={t("egGj01ab1234")} type="text" required={formVehicleType !== 'None / Walk-in'} />
              </div>
              <div className="space-y-1 md:col-span-2">
                <label className="text-label-sm font-label-sm text-on-surface-variant">{t('darshanDateLabel')}</label>
                <input value={formDate} onChange={e => setFormDate(e.target.value)} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2.5 text-body-md focus:ring-primary focus:border-primary outline-none" type="date" />
              </div>
              
              {/* Dynamic Visitor Fields */}
              <div className="md:col-span-2 space-y-4 border-t border-outline-variant pt-4">
                <h4 className="text-sm font-bold text-primary">{t("visitorDetails")}</h4>
                {Array.from({
                length: formPersons
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-xs font-semibold text-on-surface-variant" htmlFor={`offline_visitor_name_${index}`}>{t("fullName")}</label>
                        <input className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2 text-body-md focus:ring-primary focus:border-primary outline-none" id={`offline_visitor_name_${index}`} name={`offline_visitor_name_${index}`} placeholder={t("enterFullName")} required type="text" value={formVisitors[index]?.name || ''} onChange={e => handleVisitorChange(index, 'name', e.target.value)} />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-semibold text-on-surface-variant" htmlFor={`offline_visitor_age_${index}`}>{t("age")}</label>
                        <input className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-4 py-2 text-body-md focus:ring-primary focus:border-primary outline-none" id={`offline_visitor_age_${index}`} name={`offline_visitor_age_${index}`} placeholder={t("enterAge")} required type="number" min="1" max="120" value={formVisitors[index]?.age || ''} onChange={e => handleVisitorChange(index, 'age', e.target.value)} />
                      </div>
                    </div>
                  </div>)}
              </div>
              ...
              <div className="md:col-span-2 pt-4">
                <button type="submit" className="w-full bg-primary text-white py-4 rounded-lg font-headline-md text-headline-md shadow-lg hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-3">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-outline-variant">
                  {entries.map(item => <tr key={item.token} onClick={() => setSelectedToken(item.token)} className={`hover:bg-surface-container-low transition-colors cursor-pointer ${item.token === selectedToken ? 'bg-primary/5 font-semibold' : ''}`}>
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-3 py-1 rounded-full text-label-sm font-label-sm ${item.status === 'Registered' ? 'bg-[#ff9328]/20 text-[#663500] font-bold' : item.status === 'In Queue' ? 'bg-[#00bbf7]/20 text-[#004760] font-bold' : 'bg-[#dcc2af]/30 text-[#554335] font-bold'}`}>
                          {item.status}
                        </span>
                      </td>
                    </tr>)}
        ...
        <div className="xl:col-span-4 space-y-stack-lg">
          ...
          {currentDevotee && <div className="bento-card p-stack-md rounded-lg bg-surface-container-lowest border-primary border-2">
              <div className="flex flex-col items-center text-center py-6 border-b border-outline-variant mb-6">
                ...
                <span className={`px-4 py-1.5 rounded-full text-label-md font-bold flex items-center gap-2 ${currentDevotee.status === 'Registered' ? 'bg-[#ff9328]/20 text-[#663500]' : currentDevotee.status === 'In Queue' ? 'bg-[#00bbf7]/20 text-[#004760]' : 'bg-[#dcc2af]/30 text-[#554335]'}`}>
                  <span className={`w-2 h-2 rounded-full ${currentDevotee.status === 'Registered' ? 'bg-primary' : currentDevotee.status === 'In Queue' ? 'bg-[#004760]' : 'bg-[#897363]'} animate-pulse`}></span>
              ...
              <div className="space-y-4 mb-8">
                ...
                {currentDevotee.visitors && currentDevotee.visitors.length > 0 && <div className="border-t border-outline-variant/30 pt-3 space-y-2">
                    <span className="text-on-surface-variant text-[11px] font-bold uppercase tracking-wider block">{t("visitorsDetails")}</span>
                    <div className="space-y-1.5">
                      {currentDevotee.visitors.map((v, i) => <div key={i} className="flex justify-between items-center text-sm font-semibold bg-surface-container-low px-3 py-1.5 rounded border border-outline-variant/35">
              ...
              <div className="flex flex-col gap-3">
                <button onClick={handleMarkEntered} className="w-full bg-primary text-white py-3 rounded-lg font-label-md flex items-center justify-center gap-2 hover:shadow-md transition-all active:scale-[0.98]">
                  <span className="material-symbols-outlined">login</span>
                  {t('markEnteredTemple')}
                </button>
                <button onClick={handleMarkInQueue} className="w-full bg-[#006688] text-white py-3 rounded-lg font-label-md flex items-center justify-center gap-2 hover:shadow-md transition-all active:scale-[0.98]">
                  <span className="material-symbols-outlined">groups</span>
                  {t('markInQueue')}
                </button>
                <button onClick={() => handlePrintPass(currentDevotee)} className="w-full bg-white border-2 border-[#82542b] text-[#82542b] py-3 rounded-lg font-label-md flex items-center justify-center gap-2 hover:bg-[#82542b]/5 transition-all active:scale-[0.98]">
      ...
      {showPassModal && registeredDevotee && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[110] flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 md:p-8 shadow-2xl border border-outline-variant animate-in fade-in zoom-in-95 duration-200 text-on-surface relative">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowPassModal(false)} className="text-on-surface-variant hover:bg-surface-container rounded-full p-1.5 transition-colors">
            ...
            <div className="relative bg-white rounded-xl shadow-lg overflow-hidden flex flex-col lg:flex-row border border-outline-variant mb-8">
              ...
              <div className="flex-grow p-6 bg-white">
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-4">
                  ...
                  <div className="col-span-2 border-t border-outline-variant/30 pt-3">
                    <p className="text-xs text-on-surface-variant mb-1">{t("visitorDetails")}</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {registeredDevotee.visitors && registeredDevotee.visitors.length > 0 ? registeredDevotee.visitors.map((v, i) => <div key={i} className="text-sm font-semibold bg-surface-container-low px-3 py-1.5 rounded-lg border border-outline-variant/30">
                            {v.name} ({v.age} yrs)
                          </div>) : <div className="text-sm font-semibold bg-surface-container-low px-3 py-1.5 rounded-lg border border-outline-variant/30">
                ...
                <div className="mt-6 pt-4 border-t border-dashed border-outline-variant flex gap-3">
                  ...
                  <button onClick={() => handlePrintPass(registeredDevotee)} className="flex-grow h-10 bg-surface-container-high text-primary px-3 rounded-lg font-semibold flex items-center justify-center gap-1 hover:bg-surface-container-highest transition-colors text-xs active:scale-[0.98]">
                    <span className="material-symbols-outlined text-sm">print</span>
                    {t('printPass')}
                  </button>
                  <button onClick={() => handleDownloadPass(registeredDevotee)} className="flex-grow h-10 bg-primary text-white px-3 rounded-lg font-semibold flex items-center justify-center gap-1 shadow-md hover:brightness-110 transition-all text-xs active:scale-[0.98]">
              ...
              <div className="bg-surface-container-low p-6 flex flex-col items-center justify-center lg:min-w-[200px] border-t lg:border-t-0 lg:border-l border-dashed border-outline-variant">
                <div className="bg-white p-2 rounded-xl shadow-inner mb-2 border border-outline-variant">
                  <img className="w-28 h-28" alt="QR code" src={getQrUrl(registeredDevotee)} />
                </div>
