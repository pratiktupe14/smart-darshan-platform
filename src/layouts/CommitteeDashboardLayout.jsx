...
export default function CommitteeDashboardLayout() {
      ...
      if (queueRes.ok) {
        ...
        setQueueList(waiting.map((q, index) => {
          ...
          const item = {
            ...
            persons: persons,
            type: 'Regular',
            checkIn: new Date(q.checkInTime || q.createdAt || Date.now()).toLocaleTimeString([], {
              hour: '2-digit',
              minute: '2-digit'
            }),
            wait: `${waitTime}m`,
  ...
  const getSidebarLinkClasses = path => {
    ...
    return `flex items-center gap-3 px-4 py-3 rounded-lg font-bold font-label-md text-sm transition-all active:scale-[0.98] ${isActive ? 'bg-secondary-container text-on-secondary-container font-bold' : 'text-on-surface-variant hover:bg-surface-container-high font-semibold'}`;
  };
  const getHeaderLinkClasses = path => {
    const isActive = location.pathname === path;
    return isActive ? 'text-primary font-bold border-b-2 border-primary font-label-md text-sm pb-1' : 'text-on-surface-variant hover:text-primary transition-colors font-label-md text-sm font-semibold';
  };
  const themeVars = {
  ...
  return <div style={themeVars} className="bg-background text-on-surface font-body-md text-body-md overflow-x-hidden min-h-screen flex flex-col">
      {/* TopNavBar */}
      <header className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-4 md:px-10 h-16 bg-surface border-b border-outline-variant">
        <div className="flex items-center gap-4">
          <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden p-2 -ml-2 text-on-surface hover:bg-surface-container-high rounded-full transition-colors" aria-label="Open Menu">
            <span className="material-symbols-outlined">menu</span>
          </button>
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-primary text-3xl">temple_hindu</span>
            <span className="font-display text-2xl font-extrabold text-primary tracking-tighter">{t("samarthDarshanPortal")}</span>
          </div>
      ...
      {/* Mobile Backdrop */}
      {isMobileMenuOpen && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-30 lg:hidden" onClick={() => setIsMobileMenuOpen(false)}></div>}

      {/* SideNavBar */}
      <aside className={`fixed left-0 top-0 h-full w-64 bg-surface-container-low pt-20 px-4 space-y-2 border-r border-outline-variant z-40 transition-transform duration-300 ease-in-out flex flex-col overflow-y-auto ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}>
        <div className="mb-6 px-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-primary flex items-center justify-center text-white font-bold">T</div>
            <div>
              <div className="font-label-md text-sm font-bold text-on-surface">{t("samarthDarshanPortalAdmin")}</div>
              <div className="flex items-center gap-1.5">
                <span className="font-label-sm text-xs text-on-surface-variant">{t("committeeMember")}</span>
        ...
        <div className="mt-auto pb-6 flex flex-col gap-2">
          <button onClick={() => setIsNewEntryOpen(true)} className="w-full bg-primary text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 saffron-glow hover:brightness-110 active:scale-[0.98] transition-all">
      ...
      {toastMessage && <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-[#1a1c1c] text-white px-6 py-3 rounded-full shadow-lg z-[100] animate-[slideUp_0.3s_ease-out_forwards]">
          <span className="font-label-md text-sm font-semibold text-white">{toastMessage}</span>
        </div>}

      {/* --- MODALS --- */}
      {/* 1. New Entry Modal */}
      {isNewEntryOpen && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl border border-outline-variant animate-in fade-in zoom-in-95 duration-200 text-on-surface">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold flex items-center gap-2">
                <span className="material-symbols-outlined text-primary">person_add</span>{t("addNewDevoteeEntry")}</h3>
              <button onClick={() => setIsNewEntryOpen(false)} className="text-on-surface-variant hover:bg-surface-container rounded-full p-1 transition-colors">
            ...
            <form onSubmit={handleCreateNewEntry} className="space-y-4">
              <div>
                ...
                <input type="text" required value={newName} onChange={e => setNewName(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-4 py-2.5 outline-none focus:border-primary transition-all text-sm font-semibold" placeholder={t("enterFullName")} />
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-on-surface-variant mb-2">{t('queueTypeLabel')}</label>
                  <select value={newType} onChange={e => setNewType(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-3 py-2.5 outline-none focus:border-primary transition-all text-sm font-semibold">
                    <option value="Regular">{t("regularQueue")}</option>
                ...
                <div>
                  ...
                  <select value={newCategory} onChange={e => setNewCategory(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant rounded-lg px-3 py-2.5 outline-none focus:border-primary transition-all text-sm font-semibold">
              ...
              <div className="pt-4 flex gap-3 justify-end">
                <button type="button" onClick={() => setIsNewEntryOpen(false)} className="px-4 py-2 border border-outline-variant rounded-lg hover:bg-surface-container-low text-xs font-bold text-on-surface-variant transition-colors">
                  {t('cancel')}
                </button>
