...
export default function AdminDashboard() {
  ...
  return <div className="px-4 md:px-10 py-8 max-w-[1600px] mx-auto w-full space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-outline-variant/30 pb-6">
        ...
        <div className="flex items-center gap-6">
          <div className="flex flex-col items-end">
            <p className="text-sm font-bold text-primary">{t("samarthDarshanPortal")}</p>
            <p className="text-[10px] uppercase tracking-wider font-bold text-on-surface-variant font-label-sm">{t("udaipurIndia")}</p>
          </div>
      ...
      {isEmergencyActive && <div className="bg-error-container text-on-error-container p-4 rounded-xl border border-error/30 flex items-center gap-3 animate-pulse">
          <span className="material-symbols-outlined text-error text-2xl">report</span>
          <div>
            <p className="font-bold text-sm">{t("emergencyProtocolActive")}</p>
            <p className="text-xs opacity-90">{t("allTempleEntrancesAreClosedSig")}</p>
          </div>
        </div>}
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant shadow-soft border-l-4 border-l-primary hover:-translate-y-1 transition-transform duration-200">
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="flex items-center gap-1 text-on-surface-variant">
              ...
              <span className="text-xs font-medium">{t("85Cap")}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant shadow-soft hover:-translate-y-1 transition-transform duration-200">
          ...
          <div className="flex items-end justify-between">
            ...
            <span className="text-xs font-bold text-primary">{t("avg22mhr")}</span>
          </div>
        </div>
        ...
        <div className="bg-surface-container-lowest p-5 rounded-xl border border-outline-variant shadow-soft hover:-translate-y-1 transition-transform duration-200">
          ...
          <div className="flex items-end justify-between">
            ...
            <span className="text-xs font-bold text-green-600">{t("83Success")}</span>
          </div>
        </div>
          ...
          <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-soft h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="bg-primary-container px-3 py-1 rounded-full text-[10px] font-bold text-on-primary-container uppercase animate-pulse">{t("liveUpdates")}</div>
              </div>

              <div className="bg-surface-container flex flex-col items-center justify-center p-6 rounded-lg mb-6 text-center">
                <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest font-label-sm mb-1">{t("currentServingToken")}</p>
                <h3 className="text-4xl font-extrabold text-primary">#T-{servingToken}</h3>
                <div className="mt-4 flex items-center gap-6">
                  <div className="text-center">
                    <p className="text-[10px] font-bold text-on-surface-variant uppercase">{t("avgWait")}</p>
                    <p className="text-lg font-bold">{t("45Mins")}</p>
                  </div>
                  <span className="w-px h-8 bg-outline-variant"></span>
                  <div className="text-center">
                    <p className="text-[10px] font-bold text-on-surface-variant uppercase">{t("throughput")}</p>
                    <p className="text-lg font-bold">{t("120hr")}</p>
                  </div>
                </div>
              </div>

              <div className="space-y-4">
                <p className="text-xs font-bold text-on-surface-variant uppercase font-label-sm">{t("next5Tokens")}</p>
                <div className="grid grid-cols-5 gap-2">
                  {nextTokens.map((token, idx) => <div key={idx} className="bg-surface-container-low p-2 rounded text-center border border-outline-variant">
                      <p className="text-[10px] font-bold opacity-50">{idx === 0 ? 'NEXT' : `+${idx + 1}`}</p>
                      <p className="text-sm font-bold">{token}</p>
                    </div>)}
                  {nextTokens.length === 0 && <p className="col-span-5 text-sm text-center text-on-surface-variant py-2">{t("noTokensWaiting")}</p>}
                </div>
                
                <div className="pt-4">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-bold text-on-surface-variant">{t("queueFlowRate")}</p>
            ...
            <button className="mt-6 w-full py-2.5 rounded-lg border border-primary text-primary font-bold text-sm hover:bg-primary/5 active:scale-98 transition-all">{t("viewFullLiveQueue")}</button>
          </div>
        </div>
          ...
          <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-soft h-full flex flex-col">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex bg-surface-container rounded-lg p-0.5">
                <button onClick={() => setAnalyticsView('daily')} className={`px-3 py-1 rounded text-xs font-bold transition-all ${analyticsView === 'daily' ? 'bg-white text-on-surface shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}>{t("daily")}</button>
                <button onClick={() => setAnalyticsView('weekly')} className={`px-3 py-1 rounded text-xs font-bold transition-all ${analyticsView === 'weekly' ? 'bg-white text-on-surface shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}>{t("weekly")}</button>
              </div>
            </div>
            ...
            <div className="flex-1 min-h-[250px] relative flex flex-col justify-end">
              ...
              <div className="flex items-end gap-4 h-48 px-4 relative z-10">
                {chartData.map((data, idx) => <div key={idx} className="flex-1 flex flex-col justify-end h-full group relative cursor-pointer">
                    <div className="flex gap-1 w-full items-end h-full">
                      <div className="flex-1 bg-primary rounded-t-md hover:opacity-90 transition-all duration-300" style={{
                    height: `${data.normal}%`
                  }} title={`Normal: ${data.normal}%`}></div>
                      <div className="flex-1 bg-primary-container opacity-50 rounded-t-md hover:opacity-75 transition-all duration-300" style={{
                    height: `${data.festival}%`
                  }} title={`Festival: ${data.festival}%`}></div>
                    </div>
                    {/* Tooltip on hover */}
              ...
              <div className="flex w-full justify-between mt-2 px-4 text-[10px] font-bold text-on-surface-variant uppercase">
                {chartData.map((data, idx) => <span key={idx} className={data.label === 'Thu' ? 'text-primary' : ''}>
                    {data.label}
                  </span>)}
              </div>
              ...
              <div className="mt-6 flex flex-wrap gap-6 justify-center">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium">{t("normalDays")}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-primary-container opacity-50"></span>
                  <span className="text-xs font-medium">{t("festivalBookings")}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-3 bg-tertiary-container/10 rounded-lg border border-tertiary/20">
                <p className="text-[10px] font-bold uppercase text-tertiary font-label-sm">{t("projectedCapacity")}</p>
                <p className="text-lg font-bold text-on-surface">{t("15000Day")}</p>
              </div>
              <div className="p-3 bg-secondary-container/10 rounded-lg border border-secondary/20">
                <p className="text-[10px] font-bold uppercase text-secondary font-label-sm">{t("slotUtilization")}</p>
        ...
        <div className="lg:col-span-8">
          <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-soft h-full">
            <div className="flex items-center justify-between mb-6">
              ...
              <button className="text-sm font-bold text-primary hover:underline">{t("viewAllLogs")}</button>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="text-xs font-bold text-on-surface-variant uppercase font-label-sm border-b border-outline-variant">
                  <tr>
                    <th className="py-3 px-2">{t("devoteeId")}</th>
                    <th className="py-3 px-2">{t("action")}</th>
                    <th className="py-3 px-2">{t("staffMember")}</th>
                    <th className="py-3 px-2">{t("gate")}</th>
                    <th className="py-3 px-2">{t("timestamp")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-outline-variant text-sm">
                  {recentActivities.length > 0 ? recentActivities.map((act, idx) => <tr key={idx} className="hover:bg-surface-container-lowest transition-colors">
                      ...
                      <td colSpan="5" className="text-center py-8 text-on-surface-variant text-sm font-medium">{t("noRecentActivitiesFound")}</td>
                    </tr>}
                </tbody>
              </table>
            </div>
        ...
        <div className="lg:col-span-4">
          <div className="bg-primary p-6 rounded-xl shadow-lg h-full text-on-primary relative overflow-hidden flex flex-col justify-between">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div>
                <h3 className="text-3xl font-extrabold mb-2">{t("ramNavami")}</h3>
                <p className="text-sm opacity-90 leading-relaxed">{t("peakEventExpectedOn17thAprilHi")}</p>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between items-center text-xs border-b border-white border-opacity-20 pb-2">
                  ...
                  <span className="font-bold">{t("50000Day")}</span>
                </div>
                <div className="flex justify-between items-center text-xs border-b border-white border-opacity-20 pb-2">
                  <span className="font-medium text-white/80">{t("advanceBookings")}</span>
                  <span className="font-bold">34,290 (68%)</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="font-medium text-white/80">{t("specialPoojaSlots")}</span>
                  <span className="font-bold text-white bg-primary-container px-2 py-0.5 rounded">{t("fullyBooked")}</span>
                </div>
              </div>
            </div>

            <div className="mt-8 flex gap-3 relative z-10">
              <button className="flex-1 bg-white text-primary font-bold py-2.5 rounded-lg text-sm shadow-sm hover:bg-opacity-90 active:scale-95 transition-all">{t("adjustCapacity")}</button>
      ...
      <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-soft">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-bold flex items-center gap-2">
            <span className="material-symbols-outlined text-secondary font-variation-settings-['FILL'_1]">badge</span>{t("committeeStaffAnalytics")}</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {staffAnalytics.length > 0 ? staffAnalytics.map((staff, idx) => <div key={idx} className="p-4 rounded-xl border border-outline-variant hover:border-secondary transition-all bg-surface flex flex-col justify-between h-full group">
              <div>
                <p className="font-bold text-on-surface text-lg group-hover:text-secondary transition-colors">{staff.name}</p>
                <div className="mt-4 flex items-center justify-between">
                  <p className="text-xs text-on-surface-variant font-bold uppercase tracking-wider">{t("entriesProcessed")}</p>
              ...
              <p className="text-[10px] text-on-surface-variant mt-4 font-bold flex items-center gap-1 border-t border-outline-variant/50 pt-2">
                ...
                Last Active: {new Date(staff.lastActive).toLocaleTimeString([], {
              hour: '2-digit',
              minute: '2-digit'
            })}
              </p>
            </div>) : <p className="text-sm text-on-surface-variant col-span-full font-medium py-4">{t("noStaffAnalyticsAvailableYetDa")}</p>}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant hover:border-primary transition-all duration-200 cursor-pointer group flex flex-col justify-between">
            ...
            <div>
              <p className="font-bold text-sm text-on-surface">{t("addBooking")}</p>
              <p className="text-[10px] text-on-surface-variant mt-1">{t("directPortalEntry")}</p>
            </div>
          </div>
          ...
          <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant hover:border-primary transition-all duration-200 cursor-pointer group flex flex-col justify-between">
            ...
            <div>
              <p className="font-bold text-sm text-on-surface">{t("addVipEntry")}</p>
              <p className="text-[10px] text-on-surface-variant mt-1">{t("manualClearance")}</p>
            </div>
          </div>
          ...
          <div className="bg-surface-container-low p-4 rounded-xl border border-outline-variant hover:border-primary transition-all duration-200 cursor-pointer group flex flex-col justify-between">
            ...
            <div>
              <p className="font-bold text-sm text-on-surface">{t("manageStaff")}</p>
              <p className="text-[10px] text-on-surface-variant mt-1">{t("rosterAttendance")}</p>
            </div>
          </div>
          ...
          <Link to="/dashboard/admin/announcements" className="bg-surface-container-low p-4 rounded-xl border border-outline-variant hover:border-primary transition-all duration-200 cursor-pointer group flex flex-col justify-between">
            ...
            <div>
              <p className="font-bold text-sm text-on-surface">{t("createAnnouncement")}</p>
              <p className="text-[10px] text-on-surface-variant mt-1">{t("appSmsAlerts")}</p>
            </div>
          </Link>
        </div>
          ...
          <div className="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-soft">
            ...
            <div className="space-y-4">
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <p className="text-sm font-bold leading-tight text-on-surface">{t("serverDelayAlert")}</p>
                  <p className="text-[10px] text-on-surface-variant mt-1">{t("verificationSystemLagDetectedA")}</p>
                  <p className="text-[10px] font-medium text-outline mt-1">{t("2MinsAgo")}</p>
                </div>
              </div>
              ...
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <p className="text-sm font-bold leading-tight text-on-surface">{t("capacityWarning")}</p>
                  <p className="text-[10px] text-on-surface-variant mt-1">{t("90OfAfternoonSlotsFilledForTom")}</p>
                  <p className="text-[10px] font-medium text-outline mt-1">{t("1HourAgo")}</p>
                </div>
              </div>
              ...
              <div className="flex gap-3 items-start opacity-60">
                ...
                <div>
                  <p className="text-sm font-bold leading-tight text-on-surface">{t("systemBackupComplete")}</p>
                  <p className="text-[10px] text-on-surface-variant mt-1">{t("weeklyDataArchiveSuccessful")}</p>
                  <p className="text-[10px] font-medium text-outline mt-1">{t("3HoursAgo")}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
          <div className="flex flex-col sm:flex-row items-center gap-4 bg-surface p-4 rounded-xl border border-outline-variant/50">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => adjustCapacity(-5000)} className="w-10 h-10 rounded bg-outline-variant/30 hover:bg-outline-variant/50 flex items-center justify-center font-bold text-lg transition-colors active:scale-90">{t("5k")}</button>
              <span className="font-extrabold text-xl w-28 text-center text-primary">
                {visitorLimit.toLocaleString()}
              </span>
              <button onClick={() => adjustCapacity(5000)} className="w-10 h-10 rounded bg-outline-variant/30 hover:bg-outline-variant/50 flex items-center justify-center font-bold text-lg transition-colors active:scale-90">{t("5k1")}</button>
            </div>
          </div>

          <div>
            <button onClick={handleEmergencyClosure} className={`w-full py-4 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md active:scale-[0.98] ${isEmergencyActive ? 'bg-on-surface hover:bg-on-surface/90' : 'bg-error hover:bg-error/95 shadow-error/20'}`}>
