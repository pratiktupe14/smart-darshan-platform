...
export default function ExportModal({ isOpen, onClose }) {
  ...
  const toggleVisualization = (id) => setSelectedVisualizations(prev => ({ ...prev, [id]: !prev[id] }));

  const sectionsList = [
    { id: 'bookings', label: 'Bookings & Darshan' },
    { id: 'queue', label: 'Queue Status' },
    { id: 'demographics', label: 'Devotees Demographics' },
    { id: 'vip', label: 'VIP Bookings' },
    { id: 'qr', label: 'QR Verification Status' },
    { id: 'completion', label: 'Darshan Completion Rate' },
    { id: 'parking', label: 'Parking Management' },
    { id: 'support', label: 'Support Requests' },
    { id: 'revenue', label: 'Revenue Summary' },
    { id: 'charts', label: 'Analytics Charts & Graphs' }
  ];

  const visualizationsList = [
    { id: 'bar', label: 'Bar Charts (Volume)' },
    { id: 'line', label: 'Line Graphs (Trends)' },
    { id: 'pie', label: 'Pie/Donut Charts (Split)' },
    { id: 'area', label: 'Area Map (Flow)' }
  ];
    ...
    try {
      ...
      if (selectedSections.bookings || selectedSections.queue || selectedSections.revenue) {
        const summaryData = [
          ...
          { Metric: 'Completed Darshans', Value: data.summary.completedDarshans },
          { Metric: 'Total VIPs', Value: data.summary.totalVIPs },
          { Metric: 'Total Support Requests', Value: data.summary.totalSupport },
          { Metric: 'Resolved Support Requests', Value: data.summary.resolvedSupport },
          { Metric: 'Pending Queue', Value: data.summary.queueStatus?.pending || 0 },
          { Metric: 'In Queue', Value: data.summary.queueStatus?.inQueue || 0 },
        ];
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.text('Samarth Darshan Portal', pageWidth / 2, 20, { align: 'center' });
      
      doc.setFontSize(16);
      doc.setTextColor(50, 50, 50);
      doc.text('Comprehensive Analytics Report', pageWidth / 2, 30, { align: 'center' });
      
      doc.setFontSize(10);
      doc.setTextColor(100, 100, 100);
      doc.text(`Generated on: ${new Date().toLocaleString()}`, pageWidth / 2, 40, { align: 'center' });
      doc.text(`Date Range: ${dateRange}`, pageWidth / 2, 45, { align: 'center' });
      ...
      if (selectedSections.bookings || selectedSections.queue || selectedSections.revenue) {
        ...
        const summaryBody = [
          ...
          ['Completed Darshans', data.summary.completedDarshans.toString()],
          ['Total VIPs', data.summary.totalVIPs.toString()],
          ['Total Support Requests', data.summary.totalSupport.toString()],
      ...
      if (selectedSections.charts) {
        ...
        if (selectedVisualizations.bar) {
          ...
          const barChartImg = await generateChartImage('bar', {
            ...
            datasets: [{ label: 'Daily Bookings', data: dailyValues.length ? dailyValues : [0], backgroundColor: 'rgba(152, 67, 0, 0.7)' }]
          }, { plugins: { title: { display: true, text: 'Daily Bookings' } }});
        ...
        if (selectedVisualizations.line) {
          ...
          const monthlyLineImg = await generateChartImage('line', {
            ...
            datasets: [{ label: 'Monthly Bookings', data: monthlyValues.length ? monthlyValues : [0], borderColor: '#984300', fill: false }]
        ...
        if (selectedVisualizations.pie) {
          const queueImg = await generateChartImage('pie', {
            ...
            datasets: [{ data: [data.summary.queueStatus?.pending || 0, data.summary.queueStatus?.inQueue || 0, data.summary.completedDarshans || 0], backgroundColor: ['#FF9933', '#3399FF', '#2D5A27'] }]
          }, { plugins: { title: { display: true, text: 'Queue Status Distribution' } }});
          chartImages.push(queueImg);
          
          if (selectedSections.vip) {
            const vipImg = await generateChartImage('pie', {
              labels: ['Regular', 'VIP'],
              datasets: [{ data: [data.charts.vipVsRegular.regular || 0, data.charts.vipVsRegular.vip || 0], backgroundColor: ['#984300', '#D4AF37'] }]
            }, { plugins: { title: { display: true, text: 'VIP vs Regular Devotees' } }});
      ...
      if (selectedSections.bookings && data.bookings.length > 0) {
        ...
        const bookingBody = data.bookings.slice(0, 50).map(b => [
          b.fullName,
          b.persons.toString(),
          new Date(b.darshanDate).toLocaleDateString(),
          b.status
        ]);
      ...
      if (selectedSections.vip && data.vipRequests.length > 0) {
        ...
        const vipBody = data.vipRequests.map(v => [
          v.tokenNumber,
          v.name,
          v.category,
          v.status
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-on-background/40 backdrop-blur-[2px]">
      <div className="relative w-full max-w-6xl max-h-[95vh] bg-surface-container-lowest rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-outline-variant transition-all duration-300">
        ...
        <div className="px-8 py-6 flex justify-between items-start border-b border-outline-variant bg-surface-container-low">
          <div>
            <h2 className="text-2xl font-extrabold text-on-surface tracking-tight">Export Reports</h2>
            <p className="text-sm text-on-surface-variant font-medium mt-1">Generate professional reports using real analytics data.</p>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row relative">
          ...
          {error && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 bg-red-50 text-red-700 px-4 py-3 rounded-lg shadow-lg flex items-start gap-2 border border-red-200">
          ...
          <div className="flex-1 p-8 space-y-8 overflow-y-auto custom-scrollbar">
            ...
            <section>
              <h3 className="text-[11px] font-extrabold text-primary tracking-[0.1em] uppercase mb-4">Export Format</h3>
              <div className="grid grid-cols-2 gap-4">
                <label className="group cursor-pointer">
                  ...
                  <div className="p-4 border-2 border-outline-variant rounded-xl peer-checked:border-primary peer-checked:bg-primary/5 transition-all flex items-center gap-4 hover:border-primary/50">
                    ...
                    <div>
                      <div className="font-bold text-on-surface">Excel Report</div>
                      <div className="text-[11px] font-medium text-on-surface-variant">.xlsx file format</div>
                    </div>
                ...
                <label className="group cursor-pointer">
                  ...
                  <div className="p-4 border-2 border-outline-variant rounded-xl peer-checked:border-primary peer-checked:bg-primary/5 transition-all flex items-center gap-4 hover:border-primary/50">
                    ...
                    <div>
                      <div className="font-bold text-on-surface">PDF Document</div>
                      <div className="text-[11px] font-medium text-on-surface-variant">Ready for print</div>
                    </div>
                    <div className="ml-auto opacity-0 peer-checked:opacity-100 text-primary">
                      <span className="material-symbols-outlined">check_circle</span>
                    </div>
            ...
            {/* Date Range */}
            <section>
              <h3 className="text-[11px] font-extrabold text-primary tracking-[0.1em] uppercase mb-4">Date Range</h3>
              <div className="flex flex-wrap gap-2 mb-4">
                {['Today', 'Yesterday', 'Last 7 Days', 'Last 30 Days', 'This Quarter'].map(r => (
                  <button 
                    key={r}
                    onClick={() => { setDateRange(r); setShowCustomDate(false); }}
              ...
              <div onClick={() => { setDateRange('Custom'); setShowCustomDate(true); }} className={`flex items-center gap-4 px-4 py-3.5 bg-surface-container-low rounded-xl border group hover:border-primary/50 transition-all cursor-pointer ${dateRange === 'Custom' ? 'border-primary' : 'border-outline-variant'}`}>
                ...
                <button className="ml-auto text-primary font-extrabold text-[11px] uppercase tracking-wider">Change</button>
              </div>

              {showCustomDate && (
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-on-surface-variant font-bold uppercase tracking-wider mb-2 block">Start Date</label>
                  ...
                  <div>
                    <label className="text-xs text-on-surface-variant font-bold uppercase tracking-wider mb-2 block">End Date</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <section>
                <h3 className="text-[11px] font-extrabold text-primary tracking-[0.1em] uppercase mb-4">Report Sections</h3>
              ...
              <section>
                <h3 className="text-[11px] font-extrabold text-primary tracking-[0.1em] uppercase mb-4">Visualizations</h3>
                <div className="grid grid-cols-1 gap-y-3">
                  {visualizationsList.map(v => (
                    <label key={v.id} className="flex items-center gap-3 cursor-pointer group">
                      <input 
                        type="checkbox" 
                        checked={selectedVisualizations[v.id]} 
                        onChange={() => toggleVisualization(v.id)} 
                        className="w-4 h-4 rounded border-outline-variant text-primary focus:ring-primary/20 transition-all" 
                      />
          ...
          <div className="w-full lg:w-[420px] bg-surface-container border-l border-outline-variant p-8 flex flex-col">
            <h3 className="text-[11px] font-extrabold text-primary tracking-[0.1em] uppercase mb-6">Report Preview</h3>
            ...
            <div className="flex-1 bg-white shadow-2xl rounded-sm aspect-[1/1.4] w-full p-8 border border-outline-variant flex flex-col scale-100 transform origin-top hover:scale-[1.01] transition-transform duration-500 overflow-hidden relative">
              <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none rotate-[-35deg] text-6xl font-black whitespace-nowrap">
                SHREE DEVI TEMPLE
              </div>

              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-primary rounded flex items-center justify-center text-white font-extrabold text-sm">SD</div>
                  <div>
                    <div className="text-[8px] font-black text-on-surface-variant/60 uppercase tracking-[0.1em]">Sacred Operations</div>
                    <div className="text-sm font-serif font-bold text-primary-variant leading-none">Sri Devi Temple</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-[8px] font-black text-primary uppercase">Performance Report</div>
                  <div className="text-[8px] text-on-surface-variant">Selected Period</div>
                </div>
              </div>
              ...
              <div className="space-y-5 relative">
                ...
                {selectedSections.charts && (
                  <div className="h-32 bg-surface-container-lowest border border-outline-variant/30 rounded-lg flex items-end justify-center p-4 gap-2 overflow-hidden">
        ...
        <div className="px-8 py-5 border-t border-outline-variant bg-surface-container-low flex justify-between items-center relative z-10">
          <button onClick={() => {
            ...
            setSelectedSections({ bookings: true, queue: true, demographics: true, vip: false, qr: false, completion: false, parking: false, support: false, revenue: true, charts: true });
            setSelectedVisualizations({ bar: true, line: true, pie: false, area: false });
          ...
          <div className="flex gap-4">
            ...
            <button 
              onClick={() => exportFormat === 'excel' ? exportExcel() : exportPDF()} 
              disabled={loading}
              className="px-8 py-2.5 rounded-xl bg-primary text-white font-extrabold text-sm shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95 disabled:opacity-50"
            >
              Generate Report
            </button>
          </div>
        ...
        <div className={`absolute inset-0 bg-surface-container-lowest/95 backdrop-blur-xl z-20 flex flex-col items-center justify-center transition-opacity duration-500 ${loading ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
          <div className="w-full max-w-md p-8 text-center">
            ...
            <h2 className="text-2xl font-extrabold text-on-surface mb-3 tracking-tight">Compiling Report</h2>
            ...
            <div className="w-full h-2.5 bg-surface-container rounded-full overflow-hidden mb-8 relative border border-outline-variant/30">
              <div className="absolute left-0 top-0 h-full bg-primary shimmer transition-all duration-700 ease-out" style={{ width: `${progressStep * 33}%` }}></div>
            </div>
            ...
            <div className="text-left space-y-5 px-4 max-w-[280px] mx-auto">
              <div className={`flex items-center gap-4 transition-all duration-300 ${progressStep >= 1 ? 'opacity-100' : 'opacity-25'}`}>
                ...
                <span className={`text-sm ${progressStep >= 1 ? 'font-extrabold' : 'font-bold'} text-on-surface`}>Loading Sacred Data</span>
              </div>
              <div className={`flex items-center gap-4 transition-all duration-300 ${progressStep >= 2 ? 'opacity-100' : 'opacity-25'}`}>
                ...
                <span className={`text-sm ${progressStep >= 2 ? 'font-extrabold' : 'font-bold'} text-on-surface`}>Generating Analytics Charts</span>
              </div>
              <div className={`flex items-center gap-4 transition-all duration-300 ${progressStep >= 3 ? 'opacity-100' : 'opacity-25'}`}>
                ...
                <span className={`text-sm ${progressStep >= 3 ? 'font-extrabold' : 'font-bold'} text-on-surface`}>Formatting Export File</span>
              </div>
            </div>
          </div>
        </div>
